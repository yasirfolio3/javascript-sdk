 ...
 * @return {string}
 */
function getErrorMessage(maybeError: any, defaultMessage?: any): string {
...
export class ProjectConfigManager {
  private __updateListeners: any;
  private __configObj: any;
  private __optimizelyConfigObj: any;
  private __readyPromise: any;
  private jsonSchemaValidator: any;
   ...
   * successful result.
   */
  __onDatafileManagerReadyFulfill(): any {
   ...
   * @param {Error} err
   */
  __onDatafileManagerReadyReject(err: any): any {
    return {
      success: false,
   ...
   * @returns {boolean}
   */
  __validateDatafileOptions(datafileOptions: any): boolean {
   ...
   */
  __handleNewDatafile(newDatafile: any): Error | null {
    ...
    if (error) {
      ...
      if (configObj && oldRevision !== configObj.revision) {
        ...
        this.__updateListeners.forEach(function(listener: any) {
          listener(configObj);
        });
      }
    }
