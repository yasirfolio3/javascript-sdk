import { BaseDispatchQueue, EntryStatus, DispatchQueueEntry } from './dispatchQueue'
...
export default class BrowserDispatchQueue extends BaseDispatchQueue {
  start(): void {
    super.start()
    debugger;
    ...
    try {
      if (data) {
        const parsed = JSON.parse(data) as DispatchQueueEntry[]
        parsed.forEach((item: DispatchQueueEntry) => {
          this.dispatch(item, () => {})
        })
      }
    } catch (e) {
