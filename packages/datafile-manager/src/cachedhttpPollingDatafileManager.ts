...
export default abstract class CachedHttpPollingDatafileManager implements DatafileManager {
  ...
  private asyncCache: ReactNativeAsyncStorageCache
  ...
  constructor(config: DatafileManagerConfig) {
    ...
    // added async storage for datafile cache
    this.asyncCache = new ReactNativeAsyncStorageCache();
    ...
    if (datafile) {
      ...
      this.asyncCache.set('datafile', datafile)
      this.resolveReadyPromise()
    } else {
      this.currentDatafile = null
      this.asyncCache.remove('datafile')
    }
  ...
  get(): object | null {
    return this.asyncCache.get('datafile').then((val: string | null) => val) || this.currentDatafile
  }
  ...
  private onRequestComplete(this: CachedHttpPollingDatafileManager): void {
    if (!this.isStarted) {
      return
    }
